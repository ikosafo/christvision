...
$pinq = $mysqli->query("select * from website_audio_sermon ORDER BY id DESC");
?>
<style>
    .dataTables_filter {
        display: none;
        ...
        <div class="table-responsive">
            <table id="data-table" class="table" style="margin-top: 3% !important;">
                ...
                while ($fetch = $pinq->fetch_assoc()) {
                    ?>
                    <tr>
                        <td><img src="../<?php echo $fetch['image_location'] ?>" width="100" height="50"/></td>
                        <td><?php echo $fetch['image_type']; ?></td>
                        <td><?php echo $fetch['image_size']; ?></td>
                        <td><?php echo $fetch['dateuploaded']; ?></td>
                        <td>
                            <button type="button"
                                    data-type="confirm"
                                    class="btn btn-danger delete_audio"
...
<script>
    ...
    $(document).off('click', '.delete_audio').on('click', '.delete_audio', function () {
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_audio.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
