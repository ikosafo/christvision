...
if ($branch == "All") {
    ...
    $sel = mysqli_query($con,"select count(*) as allcount from `convert`");
...
## Fetch records
    $empQuery = "select * from `convert` WHERE id != '' AND 1 ".$searchQuery." ORDER BY
...
## Total number of records without filtering
    $sel = mysqli_query($con,"select count(*) as allcount FROM `convert` WHERE branch = '$branch'");
    $records = mysqli_fetch_assoc($sel);
    $totalRecords = $records['allcount'];

## Total number of record with filtering
    $sel = mysqli_query($con,"select count(*) as allcount FROM `convert` WHERE branch = '$branch' AND 1 ".$searchQuery);
    $records = mysqli_fetch_assoc($sel);
    $totalRecordwithFilter = $records['allcount'];

## Fetch records
    $empQuery = "select * FROM `convert` WHERE branch = '$branch'AND 1 ".$searchQuery." ORDER BY full_name limit ".$row.",".$rowperpage;
    $empRecords = mysqli_query($con, $empQuery);
    $data = array();
}
