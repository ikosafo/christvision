...
$pinq = $mysqli->query("select * from mnotify where branch = '$branch'");
?>
<style>
    .dataTables_filter {
        display: none;
...
<script>
    ...
    $(document).off('click', '.edit_smskey').on('click', '.edit_smskey', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.ajax({
            type: "POST",
            url: "ajax/forms/editform_smskey.php",
            data: {
                i_index: theindex
            },
            dataType: "html",
    ...
    $(document).off('click', '.delete_smskey').on('click', '.delete_smskey', function () {
        var theindex = $(this).attr('i_index');
        //alert(theindex)
        $.confirm({
            title: 'Delete Key!',
            ...
            buttons: {
                ...
                yes: {
                    ...
                    action: function () {
                        $.ajax({
                            type: "POST",
                            url: "ajax/queries/delete_smskey.php",
                            data: {
                                i_index: theindex
                            },
                            dataType: "html",
